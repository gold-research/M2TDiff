...
try:
    from torchvision.ops import RoIAlign as _TorchRoIAlign
    _HAS_ROI_ALIGN = True
except Exception:
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=4,
        ...
        self.TDAM = False
        self.use_mgte = use_mgte
        self.use_smtd = use_smtd
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, class_embed = None ):
        ...
        if self.two_stage:
            ...
            # RDQG already provides per-frame content-aware embeddings [bs, N, 2C]
        ...
        # decoder
        hs, inter_references, decoder_route_infos = self.decoder(tgt, reference_points, memory,
                                            spatial_shapes, level_start_index, valid_ratios, query_embed, mask_flatten)

        inter_references_out = inter_references
        if self.two_stage:
            return hs, init_reference_out, inter_references_out, enc_outputs_class, enc_outputs_coord_unact, None, None, {}
        ...
        final_hs, final_references_out, temporal_route_infos = self.temporal_decoder(cur_hs, cur_reference_out, cur_memory,
                                        spatial_shapes[0:1], level_start_index[0:1], valid_ratios[0:1], None, None)

        # SMTD: expose per-layer routing info for the load-balance loss
        aux_losses = {}
        if self.use_smtd:
            aux_losses['smtd_route_infos'] = (decoder_route_infos, temporal_route_infos)

        return hs[:,0:1,:,:], init_reference_out[0:1], inter_references_out[:,0:1,:,:], None, None, final_hs, final_references_out, aux_losses
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        # MGTE: MS-DGC graph branch in parallel with MHDA
        self.use_mgte = use_mgte
        if use_mgte:
            from .mgte import DynamicGraphConv
            self.ms_dgc = DynamicGraphConv(d_model, num_layers=graph_layers, knn_k=knn_k)
            # learnable soft-blending: initialized toward the MHDA baseline
            self.fusion_coef = nn.Parameter(torch.tensor(3.0))
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        ...
        if self.use_mgte:
            # MS-DGC graph branch (per-scale dynamic graph convolution)
            src_g = self.ms_dgc(src, spatial_shapes, level_start_index, padding_mask)
            alpha = torch.sigmoid(self.fusion_coef)
            src = alpha * src + (1 - alpha) * src_g
            src = self.norm_fusion(src)
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        # ffn -> QMB: Sparsely-Gated MoE block replacing the FFN
        self.use_smtd = use_smtd
        if use_smtd:
            from .smtd import QMB
            self.qmb = QMB(d_model, d_ffn, num_experts, dropout, activation)
        else:
            self.linear1 = nn.Linear(d_model, d_ffn)
            self.activation = _get_activation_fn(activation)
            self.dropout3 = nn.Dropout(dropout)
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None):
        ...
        # ffn / QMB
        if self.use_smtd:
            tgt, route_info = self.qmb(tgt)
            tgt = self.norm3(tgt)
            return tgt, route_info
...
class TemporalDeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output, route_info = layer(output, query_pos, reference_points_input, src, src_spatial_shapes, src_level_start_index, src_padding_mask)
        ...
        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(intermediate_reference_points), route_infos

        return output, reference_points, route_infos  
...
def build_deforamble_transformer(args):
    return DeformableTransformer(
        ...
        use_mgte=getattr(args, 'use_mgte', False),
        graph_layers=getattr(args, 'graph_layers', 2),
        knn_k=getattr(args, 'knn_k', 11),
