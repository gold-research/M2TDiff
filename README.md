...
M2TDiff is evaluated on the widely used video object detection benchmark, **ImageNet VID**. To further evaluate its generalization capability, we additionally conduct experiments on **VisDrone-VID**. Before training and evaluation,  we convert the JSON files of the two datasets using `tools/convert_to_vid_json.py`.
